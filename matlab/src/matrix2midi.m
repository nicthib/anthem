...
for i=1:Ntracks
  ...
  for j=1:length(ord)
    ...
    if (note_events_onoff(ord(j))==1)
      ...
      if sustainon
          midi.track(i).messages(msgCtr+1) = midi.track(i).messages(msgCtr);
          midi.track(i).messages(msgCtr+1).chan = 0;
          midi.track(i).messages(msgCtr+1).type = 176;
          midi.track(i).messages(msgCtr+1).data = [64; 127];
          midi.track(i).messages(msgCtr+1).deltatime = 0;
      end
    else
      %-- note off msg:
      ...
      if sustainon
          ...
          midi.track(i).messages(msgCtr+1).chan = 0;
          midi.track(i).messages(msgCtr+1).type = 176;
          midi.track(i).messages(msgCtr+1).data = [64; 0];
          midi.track(i).messages(msgCtr+1).deltatime = 0;
      end
    end
    if sustainon
